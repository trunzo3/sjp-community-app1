...
- **My Plan (Safety Plan)**: Private safety planning tool for clients and alumni only. Includes 5 editable sections (warning signs, trusted people, safe places, coping strategies, reasons to keep going) plus static crisis contacts. Auto-saves on blur with silent retry. One plan per user, upserted. Stored in `safety_plans` table. Privacy enforced at API level — userId always from session, no staff/admin override. Staff/admin cannot see the entry point or access the page. Route: `/my-plan`. Components: `client/src/pages/my-plan.tsx`.
