...
- **My Journey Progress Dashboard**: Visual progress tracking for clients/alumni across six categories (Journey/104, Employment/11, Housing/8, Finance/8, Parenting/12, Community/4). Overall % = total completed sessions / 147. Clients can self-edit progress via tap-to-edit slider view. Staff/admin edit via Admin Panel. Database uses `category` enum (separate from resource `resourceCategoryEnum`).
- **Resources**: Categorized under 8 practical categories via `resource_category` enum: jobs, housing, childcare, transportation, health, money, legal, education (replaces old pillar-based categorization). Each category has a distinct filter color. DB column is still named `pillar` for backward compat but typed as `resource_category`.
...
- **Engagement Streak Tracking**: Private, personal streak system that recognizes consistent app usage. Tracks qualifying interactions (affirmation tap, posting, reacting, replying, event detail view, resource tap, profile visit) via `user_activity` table with unique (userId, activityDate) constraint. Streak calculated server-side on new activity records. Milestone acknowledgments (3, 7, 14, 30, 60, 90, 180, 365 days) stored in `streak_acknowledgments` table and shown once as a gentle banner on the home screen above the affirmation card. Users table has `currentStreak` and `longestStreak` fields. Nothing is public, no penalties for broken streaks, no notifications. If milestone is hit again after rebuild, acknowledgment fires again. Content pending SJP review.
...
- **My Plan (Safety Plan)**: Private safety planning tool for clients and alumni only. Includes 5 editable text sections (warning signs, trusted people, safe places, coping strategies, reasons to keep going) plus an "If I need help now" section where users can add up to 5 personal helpline contacts (name/org + phone number) that render as tappable tel: links. Helpline contacts are added one at a time via an inline form (not blank fields). Auto-saves on blur/save with silent retry. One plan per user, upserted. Stored in `safety_plans` table (helplineContacts stored as JSON string). Privacy enforced at API level — userId always from session, no staff/admin override (403). Staff/admin cannot see the entry point or access the page. Route: `/my-plan`. Components: `client/src/pages/my-plan.tsx`.
